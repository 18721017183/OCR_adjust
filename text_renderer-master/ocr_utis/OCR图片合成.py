...
number_classes_dir = r'C:\Users\pc\Desktop\tmp\test\characters'   #数字图片文件夹
#所有标签
...
if flag == 'train':
    image_save_path = r'C:\Users\pc\Desktop\tmp\test\train'    #合成图片保存的文件夹，train // test
    label_txt = r'C:\Users\pc\Desktop\tmp\test\train_label.txt'      #图片的标签 train.txt // test.txt
    generate_image_num = 200
elif flag == 'test':
...
for idx in range(generate_image_num):
    ...
    for i in range(12):
        ...
        #图片旋转
        angels = np.random.randint(-5, 5)
        image = rotate_bound(dic_all_classes[a][b],angels)
        image = cv.resize(image,(w,h))
        image = image.reshape((h,w))

        #将数字图片与背景融合
        if start_width+w >= 280:
            FLAG = True
            continue
        image2[0:32,start_width:start_width+w] = image
    ...
    image_save_full_name = os.path.join(image_save_path,str(idx)+'.jpg')
    print(idx)
    cv.imwrite(image_save_full_name, image2)
    with open(label_txt,'a') as f:
        line = str(idx)+'_cut.jpg' + ' ' + words + '\n'       #添加图片名称后缀
        f.write(line)
